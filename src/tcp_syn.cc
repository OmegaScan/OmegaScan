...
int tcp_syn(string host, unsigned short port, unsigned short local_port) {
  ...
  if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &opt_val, sizeof(int)) <
      0) {
  ...
  int send_ret = sendto(sock_fd, send_buf, ip_header.tot_len, 0,
                        (struct sockaddr *)&target_addr, sizeof(target_addr));
  if (send_ret <= 0) return error_type::SOCKET_SEND_ERROR;
  ...
  do {
    ...
    recv_ret = recvfrom(sock_fd, recv_buf, MAX_PACKET_LENTH, 0,
                        (struct sockaddr *)&from_addr, &from_len);
    recv_iph = (struct iphdr *)recv_buf;
    recv_tcph = (struct tcphdr *)(recv_buf + 4 * (recv_iph->ihl));
  } while (retry++ < RETRY_TIMES && recv_ret != 1 &&
           (ntohs(recv_tcph->th_sport) != ntohs(tcp_header.th_dport) ||
            ntohl(recv_iph->saddr) != ntohl(ip_header.daddr)));
  // For resource saving
  close(sock_fd);
