...
std::vector<unsigned short> parse_port(const std::string &raw_port_specified) {
    ...
    if (start_port == std::stoi(LAST_PORT) && str_start_port != LAST_PORT) return {};
    else if (end_port == std::stoi(LAST_PORT) && str_end_port != LAST_PORT) {
        if (str_end_port == "-1") port_list.push_back(start_port);
        else return {};
    } else {
        if (end_port < start_port) return {};
        else {
            port_list.push_back(start_port);
            for (unsigned short u_item = start_port + 1; u_item <= end_port && u_item != 0; ++u_item) {
